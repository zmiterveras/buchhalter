#!/usr/bin/env python3
# -*- coding: utf-8 -*-

class MenuLanguages:
    en = {
        'add': 'Add',
        'amuse': 'Amusements',
        'bar_graph': 'Bar graph',
        'bar_graph_detailed': 'Bar graph detailed',
        'bonus': 'Bonuses',
        'category': 'Category',
        'change_expense': 'Change Expense',
        'change_income': 'Change Income',
        'choose': 'Choose',
        'close': 'Close',
        'clothes': 'Clothes',
        'current_balance': 'Current balance:',
        'data_type': 'Data type',
        'date': 'Date',
        'day': 'Day',
        'delete': 'Delete',
        'delete_record': 'Delete Record',
        'diagram': 'Diagram',
        'edit': 'Edit',
        'edit_record': 'Edit record',
        'expense': 'Expenses',
        'fit_beauty': 'Fitness and beauty',
        'food': 'Food',
        'gift': 'Gift',
        'graphics': 'Graphics',
        'half_year': 'Half-Year',
        'household': 'Household',
        'income': 'Incomes',
        'internet': 'Internet',
        'medicine': 'Medicine',
        'mobile': 'Mobile',
        'month': 'Month',
        'month_view': 'Month View',
        'new_expense': 'New expense',
        'new_income': 'New income',
        'none': 'None',
        'note': 'Note',
        'note_with': 'With Category',
        'note_without': 'Without Category',
        'office': 'Office',
        'other': 'Other',
        'percent': 'Percents',
        'perfume': 'Perfume',
        'pets': 'Pets',
        'rest': 'Rest',
        'salary': 'Salary',
        'select': 'Select',
        'select_period_table': 'Select period and data type:',
        'selected_period': 'Selected Period',
        'settings': 'Settings',
        'simple_view': 'Simple View',
        'start_date': 'Start Date',
        'stop_date': 'Stop Date',
        'sum': 'Sum:',
        'television': 'Television',
        'transfer': 'Transfer',
        'transport': 'Transport',
        'utility': 'Utility',
        'viewing': 'Viewing',
        'warning': 'Warning',
        'warn_change_text': 'Do you really want to %s record',
        'warn_select_record': 'No record selected',
        'warn_change': 'change',
        'warn_date': 'Invalid date received',
        'warn_dates': 'Stop date is older than start date',
        'warn_delete': 'delete',
        'warn_same_dates': 'Same dates',
        'week': 'Week',
        'year': 'Year',
        'year_month':'year.month'
    }
    ru = {
        'add': 'Добавить',
        'amuse': 'Развлечения',
        'bar_graph': 'Столбчатая диаграмма',
        'bar_graph_detailed': 'Столбчатая диаграмма детальная',
        'bonus': 'Премия',
        'category': 'Категория',
        'change_expense': 'Изменить расход',
        'change_income': 'Изменить доход',
        'choose': 'Выбрать',
        'close': 'Закрыть',
        'clothes': 'Одежда',
        'current_balance': 'Текущий балланс:',
        'data_type': 'Тип данных',
        'date': 'Дата',
        'day':'День',
        'delete': 'Удалить',
        'delete_record': 'Удалить запись',
        'diagram': 'Диаграмма',
        'edit': 'Редакт.',
        'edit_record': 'Редакт.запись',
        'expense': 'Расходы',
        'fit_beauty': 'Фитнес и красота',
        'food': 'Еда',
        'gift': 'Подарки',
        'graphics': 'Графики',
        'half_year': 'Полугодие',
        'household': 'Бытовое',
        'income': 'Доходы',
        'internet': 'Интернет',
        'medicine': 'Медицина',
        'mobile': 'Мобильный',
        'month': 'Месяц',
        'month_view': 'Месяц',
        'new_expense': 'Новые расход',
        'new_income': 'Новый доход',
        'none': 'Нет',
        'note': 'Примечание',
        'note_with': 'С категорией',
        'note_without': 'Без категории',
        'office': 'Канцелярия',
        'other': 'Другое',
        'percent': 'Проценты',
        'perfume': 'Парфюмерия',
        'pets': 'Домашние животные',
        'rest': 'Остаток',
        'salary': 'Зарплата',
        'select': 'Выбрать',
        'select_period_table': 'Выберите период и тип данных:',
        'selected_period': 'Выбранный период',
        'settings': 'Настройки',
        'simple_view': 'Балланс',
        'start_date': 'Начальная дата',
        'stop_date': 'Конечная дата',
        'sum': 'Итого:',
        'television': 'Телевидение',
        'transfer': 'Трансфер',
        'transport': 'Транспорт',
        'utility': 'Коммунальные',
        'viewing': 'Просмотр',
        'warning': 'Предупреждение',
        'warn_change_text': 'Вы действительно хотите %s запись',
        'warn_select_record': 'Не выбрана запись',
        'warn_change': 'изменить',
        'warn_date': 'Получена недопустимая дата',
        'warn_dates': 'Конечная дата старше начальной даты',
        'warn_delete': 'удалить',
        'warn_same_dates': 'Одинаковые даты',
        'week': 'Неделя',
        'year': 'Год',
        'year_month': 'год.месяц'
    }
    cat_keys_credit = ['other',
                'amuse',
                'clothes',
                'fit_beauty',
                'food',
                'household',
                'internet',
                'medicine',
                'mobile',
                'office',
                'perfume',
                'pets',
                'television',
                'transfer',
                'transport',
                'utility']

    cat_keys_debit = ['other',
                      'salary',
                      'bonus',
                      'gift',
                      'percent']

